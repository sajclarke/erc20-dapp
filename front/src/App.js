...
class App extends Component {
  ...
  getBalance = async (web3) => {
    ...
    //   console.log(result);
    // });
    const decimals = await tutorialInstance.methods.decimals().call({ from: accounts[0] })
    // console.log(decimals)

    // console.log(tutorialInstance)
    //We use web3.utils.fromWei to display the units of the balance from wei to ether
    this.setState({ loading: false, web3: web3, account: accounts[0], decimals: decimals, balance: web3.utils.fromWei(ttBalance, "ether"), contractInstance: tutorialInstance })
  ...
  handleFieldChange = (event)=> {
    ...
    this.setState((prevState) => {
      return {
        transferForm: { ...prevState.transferForm, [event.target.name]: event.target.value },
      }
    })
  }
  ...
  handleTransfer = (e)=>{
    ...
    contractInstance.methods.transfer(transferForm.address, web3.utils.toWei(transferForm.amount, 'ether')).send({ from: account, gas: 1000000 })
      .then((result) => {
            ...
            <Card>
              ...
              <Form onSubmit={(e) => this.handleTransfer(e)}>
                  <FormGroup>
                    ...
                    <Input type="text" name="amount" value={transferForm.amount} onChange={(e)=>this.handleFieldChange(e)} />
                  </FormGroup>
                  <FormGroup>
                    <Label for="transferAddress">Address</Label>
                    <Input type="text" name="address" value={transferForm.address}  onChange={(e)=>this.handleFieldChange(e)} />
                  </FormGroup>
